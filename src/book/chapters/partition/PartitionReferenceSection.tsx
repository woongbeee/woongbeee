...
const T = {
  ko: {
    title: 'Reference 파티션',
    subtitle:
      'Reference 파티션은 부모 테이블의 FK(Foreign Key)를 통해 자식 테이블이 부모 테이블의 파티셔닝 전략을 자동으로 상속받아요. 자식 테이블에 파티션 키 컬럼을 별도로 저장할 필요가 없어요.',

    whatTitle: 'Reference 파티션이란?',
    whatDesc:
      '주문(orders)과 주문상세(order_items)처럼 부모-자식 관계의 테이블에서, 자식 테이블이 부모 테이블의 파티셔닝 방식을 그대로 따르도록 해요. 부모 테이블에 새 파티션이 추가되면 자식 테이블에도 자동으로 해당 파티션이 생성돼요.\n\n기존 방식에서는 자식 테이블에도 order_date 같은 파티션 키 컬럼을 중복 저장해야 했어요. Reference 파티셔닝은 이 중복을 없애고, 부모-자식 파티션 간 자동 동기화를 보장해줘요.',
...
WHERE  table_name = 'ORDER_ITEMS';`,
    ...
    summary:
      'Reference 파티션은 마스터-디테일 테이블이 동일한 파티션 전략을 공유해야 할 때 가장 적합해요. 파티션 키 중복 저장을 없애고, 부모-자식 파티션 구조를 자동으로 동기화해서 관리 오버헤드를 최소화해요.',
  },
