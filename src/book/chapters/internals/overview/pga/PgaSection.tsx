...
const T = {
  ko: {
    title: 'PGA',
    subtitle: '서버 프로세스가 단독으로 사용하는 비공유 메모리 영역',

    whatTitle: 'PGA란?',
    whatP1: 'PGA(Program Global Area)는 하나의 서버 프로세스 또는 백그라운드 프로세스가 독점적으로 사용하는 비공유 메모리 영역입니다. SGA가 모든 세션이 공유하는 메모리라면, PGA는 프로세스마다 독립적으로 할당되는 전용 작업 공간입니다.',
    whatP2: '파일 담당자가 카운터 한쪽을 비워 임시 작업대로 쓰는 것처럼, 서버 프로세스는 SQL을 처리하는 동안 PGA를 사용하고 세션이 끊기면 OS에 반환합니다. 다른 프로세스는 이 공간에 접근할 수 없습니다.',
    whatP3: '인스턴스 전체의 PGA 합산을 Instance PGA라고 하며, 초기화 파라미터 PGA_AGGREGATE_TARGET으로 전체 크기 목표치를 설정합니다. 개별 PGA 크기는 직접 설정할 수 없고, Oracle이 자동으로 조정합니다.',
    ...
    components: [
      {
        ...
        desc: '파싱된 SQL과 세션별 실행 정보를 저장하는 영역. Persistent Area(바인드 변수 값)와 Runtime Area(실행 상태)로 구성됩니다.',
      },
      {
        icon: 'sort',
        color: 'blue' as const,
        label: 'SQL Work Area',
        desc: 'ORDER BY, GROUP BY(Sort Area), Hash Join(Hash Area), 복합 비트맵 인덱스 스캔(Bitmap Merge Area) 등 메모리 집약적 연산에 사용되는 영역입니다.',
      },
      {
        icon: 'db',
        color: 'teal' as const,
        label: 'UGA (User Global Area)',
        desc: '세션 변수, 로그인 정보, 세션 상태를 저장합니다. Dedicated Server 모드에서는 PGA 안에 위치하고, Shared Server 모드에서는 SGA(Large Pool)로 이동합니다.',
      },
    ],

    privateSqlTitle: 'Private SQL Area 구조',
    privateSqlDesc: 'Private SQL Area는 하나의 커서(cursor)가 사용하는 개인 공간으로, 두 부분으로 나뉩니다.',
    persistentLabel: 'Persistent Area',
    persistentDesc: '바인드 변수 값을 저장합니다. 커서가 닫힐 때 해제됩니다.',
    runtimeLabel: 'Runtime Area',
    runtimeDesc: '쿼리 실행 상태(현재까지 읽은 행 수, 페치 위치 등)를 저장합니다. DML의 경우 실행 요청 첫 단계에서 생성되고, SQL이 닫히면 해제됩니다.',
    privateSqlNote: '같은 SQL을 20개의 세션이 각각 실행하면 SGA의 Shared SQL Area(실행 계획)는 1개지만, Private SQL Area는 세션마다 1개씩 20개가 생깁니다. OPEN_CURSORS 파라미터로 클라이언트 하나가 열 수 있는 Private SQL Area 수를 제한합니다.',

    workAreaTitle: 'SQL Work Area',
    workAreaDesc: 'Work Area는 메모리 집약적인 SQL 연산을 처리하는 PGA 내 전용 공간입니다. 메모리가 부족하면 임시 테이블스페이스(Temp)로 스필(spill)이 발생합니다.',
    workAreas: [
      { icon: 'sort',   color: 'blue' as const,    label: 'Sort Area',         desc: 'ORDER BY, GROUP BY, ROLLUP, 인덱스 빌드 등 정렬 연산에 사용' },
      { icon: 'merge',  color: 'emerald' as const,  label: 'Hash Area',         desc: 'Hash Join에서 작은 테이블(build input)을 해시 테이블로 만들 때 사용' },
      { icon: 'bitmap', color: 'amber' as const,    label: 'Bitmap Merge Area', desc: '여러 Bitmap Index 스캔 결과를 병합할 때 사용' },
    ],
    workAreaNote: 'Work Area 크기는 PGA_AGGREGATE_TARGET 설정 시 Oracle이 자동으로 관리합니다(자동 PGA 관리). V$SQL_WORKAREA_ACTIVE 뷰로 현재 활성 Work Area를 확인할 수 있습니다.',
    spillTitle: 'Work Area 스필(Spill)',
    spillDesc: 'Work Area가 필요한 메모리보다 작으면 Oracle은 중간 결과를 임시 테이블스페이스에 씁니다. 이를 "디스크로 스필"이라고 하며, 성능이 크게 저하됩니다.',
    spillModes: [
      { label: 'Optimal',    color: 'emerald' as const, desc: 'Work Area가 충분 → 전부 메모리에서 처리. 가장 빠름.' },
      { label: 'One-Pass',   color: 'amber' as const,   desc: 'Work Area 부족 → 데이터를 한 번만 Temp에 씀. 중간 성능.' },
      { label: 'Multi-Pass', color: 'rose' as const,    desc: 'Work Area 매우 부족 → 여러 번 Temp를 읽고 씀. 매우 느림.' },
    ],

    dedicatedSharedTitle: 'Dedicated Server vs Shared Server',
    dedicatedSharedDesc: '서버 모드에 따라 PGA 구조와 메모리 위치가 달라집니다.',

    paramsTitle: '주요 파라미터',
    params: [
      { name: 'PGA_AGGREGATE_TARGET', desc: '인스턴스 전체 PGA의 목표 크기. Oracle이 개별 Work Area 크기를 이 범위 안에서 자동 조정합니다.' },
      { name: 'PGA_AGGREGATE_LIMIT',  desc: 'PGA 사용량의 하드 한도. 초과 시 가장 많이 사용하는 세션의 SQL이 강제 종료됩니다.' },
    ...
    monitors: [
      ...
      { name: 'V$PGA_TARGET_ADVICE',    desc: 'PGA_AGGREGATE_TARGET 값 변경 시 예상 성능 영향' },
    ...
    summaryItems: [
      '프로세스 전용 비공유 메모리 — SGA와 달리 다른 프로세스가 접근 불가',
      'Private SQL Area: 바인드 변수(Persistent) + 실행 상태(Runtime) 저장',
      'SQL Work Area: 정렬·해시 조인·비트맵 병합 등 메모리 집약 연산 처리',
      'Work Area 부족 시 Temp 스필 → Optimal > One-Pass > Multi-Pass 순 성능',
      'PGA_AGGREGATE_TARGET으로 인스턴스 전체 PGA 크기를 자동 관리',
    ],
  },
