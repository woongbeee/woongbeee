...
const T = {
  ko: {
    title: '파티셔닝된 인덱스',
    subtitle:
      '파티셔닝된 테이블에는 두 종류의 인덱스를 사용할 수 있어요. Local 인덱스는 테이블 파티션과 1:1로 대응하며, Global 인덱스는 테이블 파티션과 독립적으로 파티셔닝돼요.',

    localTitle: 'Local 파티셔닝 인덱스',
    localDesc:
      'Local 인덱스는 테이블 파티션과 1:1 대응해요. 각 인덱스 파티션은 해당 테이블 파티션의 행만 가리켜요. 테이블 파티션 추가·삭제 시 인덱스 파티션도 자동으로 동기화되어 관리가 편해요.',
...
CREATE INDEX sales_amt_idx ON sales (amount) LOCAL (
  ...
  PARTITION p_future TABLESPACE idx_ts4
    ...
    globalDesc:
      'Global 인덱스는 테이블 파티션과 독립적으로 파티셔닝돼요. 하나의 Global 인덱스 파티션이 여러 테이블 파티션의 행을 가리킬 수 있어요. 비파티셔닝 테이블에 Global 파티셔닝 인덱스를 생성하는 것도 가능해요.',
...
-- Oracle은 이를 일반 전역 인덱스로 처리함`,
    ...
    globalNote:
      '테이블 파티션이 추가·삭제·이동될 때 Global 인덱스는 자동으로 Rebuild되지 않아요. ALTER TABLE ... UPDATE GLOBAL INDEXES 옵션으로 파티션 연산과 동시에 인덱스를 유효 상태로 유지할 수 있어요.',
    ...
    partialDesc:
      'Oracle 12c부터 파티션 단위로 인덱싱을 선택적으로 활성화할 수 있어요. 최신 파티션만 인덱싱하고 오래된 아카이브 파티션은 인덱스를 제외해서 저장소와 인덱스 유지 비용을 줄여요.',
...
LOCAL INDEXING PARTIAL;  -- INDEXING ON 파티션에만 인덱스 생성`,
    ...
    summary:
      'Local 인덱스는 파티션 유지보수가 자동 동기화되어 관리 편의성이 높아요. Global 인덱스는 파티션 키와 무관한 컬럼으로 빠른 단일 행 조회가 필요할 때 사용해요. 12c 이상에서는 Partial 인덱스로 최신 파티션만 인덱싱해서 비용을 절감할 수 있어요.',
  },
