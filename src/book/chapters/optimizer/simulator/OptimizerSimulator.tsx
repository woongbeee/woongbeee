...
------- ------  ----  -------  ----  -----  -------  ----
total        3  0.00     0.00     0      2        0     1`,
    reasonKo: `EMPLOYEE_ID는 EMPLOYEES 테이블의 Primary Key입니다.
PK 컬럼에는 Oracle이 자동으로 Unique 인덱스(EMP_EMP_ID_PK)를 생성합니다.

CBO는 등치 조건(= 100)으로 PK 인덱스를 조회하면
정확히 0 또는 1건만 반환됨을 알기 때문에 INDEX UNIQUE SCAN을 선택합니다.
Index Range Scan이 아닌 Unique Scan인 이유는 Unique 인덱스에 = 조건이기 때문입니다.

실행 순서: ② INDEX UNIQUE SCAN → ① TABLE ACCESS BY INDEX ROWID → ⓪ SELECT STATEMENT
  인덱스에서 ROWID 1개를 꺼내 테이블 블록 1개만 접근합니다.
  consistent gets = 2 (인덱스 1 + 테이블 1), physical reads = 0 (캐시 히트).

이것이 Oracle에서 가장 효율적인 단건 조회 패턴입니다.`,
...
This is the most efficient single-row lookup pattern in Oracle.`,
    callStatsAnnotations: [
      { line: 'Parse  1  0.00  0.00  0  0  0  0', noteKo: 'Parse 1회. CPU·Elapsed 모두 0.00초 → Library Cache에 이미 계획이 있어 Soft Parse가 발생했습니다. Disk·Query 모두 0 — 파싱 단계에서는 데이터 블록을 읽지 않습니다.', noteEn: 'Parse ran once. Both CPU and Elapsed are 0.00s → the plan was already in the Library Cache (Soft Parse). Disk and Query are 0 — no data blocks are read during parse.' },
      { line: 'Fetch  1  0.00  0.00  0  2  0  1', noteKo: 'Fetch 1회 호출로 1행 반환. Query=2 → 논리 블록 2개 읽음(인덱스 1 + 테이블 1). Disk=0 → 모두 Buffer Cache 히트. 단건 PK 조회에서 이 이상 효율적인 수치는 없습니다.', noteEn: '1 Fetch call returned 1 row. Query=2 → 2 logical blocks read (1 index + 1 table). Disk=0 → full Buffer Cache hit. You cannot get more efficient than this for a single-row PK lookup.' },
    ],
    predicateAnnotations: [
      { line: '2 - access("EMPLOYEE_ID"=100)', noteKo: 'Op 2(INDEX UNIQUE SCAN)에서 EMPLOYEE_ID=100이 access 조건입니다. access는 인덱스의 탐색 시작/종료 범위를 결정한다는 뜻으로, 인덱스 블록을 수직으로 내려가 정확히 이 값을 찾습니다. filter가 아닌 access인 이유: 인덱스의 선두(유일) 컬럼에 등치 조건이므로 인덱스 구조 자체로 위치를 결정할 수 있기 때문입니다.', noteEn: 'Op 2 (INDEX UNIQUE SCAN) uses EMPLOYEE_ID=100 as an access predicate. access means the predicate determines the index scan range — Oracle traverses the B-tree vertically to locate this exact value. It is access (not filter) because an equality condition on the leading (unique) column lets the index structure pinpoint the entry directly.' },
    ],
    projectionAnnotations: [
      { line: '1 - output: [EMPLOYEE_ID, FIRST_NAME, LAST_NAME, SALARY]', noteKo: 'Op 1(TABLE ACCESS)이 최종적으로 SELECT 절에 필요한 4개 컬럼을 출력합니다. Oracle은 필요한 컬럼만 프로젝션하므로 테이블에서 모든 컬럼을 읽지 않습니다.', noteEn: 'Op 1 (TABLE ACCESS) projects exactly the 4 columns needed by the SELECT clause. Oracle reads only what is needed — this is column pruning in action.' },
    ...
    statsAnnotations: [
      { line: 'consistent gets      =  2', noteKo: '논리 읽기 2블록. 인덱스 1블록 + 테이블 1블록. PK 단건 조회의 이론적 최솟값입니다.', noteEn: '2 logical block reads: 1 index block + 1 table block. This is the theoretical minimum for a PK single-row lookup.' },
      { line: 'physical reads       =  0', noteKo: '물리 읽기 0. 2개 블록 모두 Buffer Cache에서 즉시 찾았습니다. 자주 조회하는 작은 테이블은 이처럼 항상 PR=0을 기대할 수 있습니다.', noteEn: 'No physical reads. Both blocks were found in the Buffer Cache immediately. For small frequently-accessed tables, PR=0 is the expected steady-state.' },
...
------- ------  ----  -------  ----  -----  -------  ----
total        3  0.00     0.00     3      8        0     6`,
    reasonKo: `SALARY 컬럼에는 인덱스가 없습니다.
인덱스 없이 WHERE SALARY > 10000 조건을 적용하려면
테이블의 모든 블록을 읽어야 합니다 → TABLE ACCESS FULL.

Predicate Information에서 filter로 표시된 것에 주목하세요.
access가 아닌 filter인 이유: 인덱스 탐색 범위를 결정하는 것이 아니라,
전체 행을 읽어온 후 조건을 적용해 버리는 방식이기 때문입니다.

설령 SALARY 인덱스가 있더라도, 전체 15행 중 6행(40%)이 조건을 만족하면
CBO는 FTS가 인덱스보다 저렴하다고 판단할 수 있습니다(소규모 테이블).
선택도가 낮은(많은 행이 반환되는) 범위 조건에서는 FTS가 정상적인 선택입니다.`,
...
Range predicates with low selectivity (many rows returned) often make FTS the cheaper choice.`,
    callStatsAnnotations: [
      { line: 'Fetch  1  0.00  0.00  3  8  0  6', noteKo: 'Disk=3 → 8개 논리 블록 중 3개가 Buffer Cache에 없어 디스크에서 읽었습니다. FTS는 처음 실행 시 PR이 높고, 재실행하면 블록이 캐시에 올라와 PR=0이 되는 것이 정상입니다.', noteEn: 'Disk=3 → 3 of the 8 logical blocks were not in the Buffer Cache and required disk reads. FTS typically has high PR on first run; a second run usually sees PR=0 as all blocks are now cached.' },
    ],
    predicateAnnotations: [
      { line: '1 - filter("SALARY">10000)', noteKo: 'Op 1(TABLE ACCESS FULL)에서 SALARY>10000이 filter입니다. filter는 "블록을 이미 읽은 후 행 단위로 조건을 검사한다"는 뜻입니다. access와 달리 스캔 범위를 좁히지 못하므로 전체 블록을 읽은 뒤 해당 조건을 만족하지 않는 행은 버려집니다.', noteEn: 'Op 1 (TABLE ACCESS FULL) uses SALARY>10000 as a filter. A filter is applied row-by-row after the block has already been read — it cannot narrow the scan range like access can. All blocks are read first; non-matching rows are discarded afterwards.' },
    ],
    projectionAnnotations: [
      { line: '1 - output: [EMPLOYEE_ID, LAST_NAME, SALARY, DEPARTMENT_ID]', noteKo: 'FTS이므로 인덱스 단계가 없고 TABLE ACCESS 단 하나만 있습니다. SELECT 절의 4개 컬럼만 프로젝션하며, 나머지 컬럼(FIRST_NAME, HIRE_DATE 등)은 읽지 않습니다.', noteEn: 'No index step — only the TABLE ACCESS operation exists. Oracle projects exactly the 4 columns in the SELECT clause; other columns (FIRST_NAME, HIRE_DATE, etc.) are not read from the row.' },
    ],
    statsAnnotations: [
      { line: 'consistent gets      =  8', noteKo: '논리 읽기 8블록. EMPLOYEES 전체 테이블 크기에 해당합니다. FTS는 인덱스 없이 모든 블록을 읽으므로 테이블 크기 = CR 값입니다.', noteEn: '8 logical block reads — the total size of the EMPLOYEES table. FTS reads every block, so CR ≈ table block count.' },
      { line: 'physical reads       =  3', noteKo: '3블록은 디스크에서 읽었습니다. 재실행하면 이 값이 0으로 줄어드는 것을 볼 수 있습니다(Buffer Cache warming).', noteEn: '3 blocks required physical disk reads. Run the query a second time and this will drop to 0 as the blocks are now in the Buffer Cache.' },
      { line: 'rows processed       =  6', noteKo: '15행 중 SALARY>10000인 6행만 최종 반환. 나머지 9행은 읽었지만 filter에서 제거되었습니다.', noteEn: '6 of 15 rows passed the SALARY>10000 filter and were returned. The other 9 rows were read from disk but discarded.' },
    ],
  },
...
------- ------  ----  -------  ----  -----  -------  ----
total        3  0.00     0.00     1      4        0     2`,
    reasonKo: `DEPARTMENT_ID는 외래키 컬럼입니다.
Oracle은 FK 컬럼에 자동으로 인덱스를 생성하지 않지만,
DBA가 EMP_DEPARTMENT_IX 인덱스를 별도로 생성한 상태입니다.

DEPARTMENT_ID = 80 등치 조건이지만 Unique 인덱스가 아니므로
(한 부서에 여러 직원이 있을 수 있음) INDEX RANGE SCAN이 선택됩니다.
UNIQUE SCAN은 Unique 인덱스에서만 사용합니다.

Predicate Information에서 2번 오퍼레이션에 access와 filter가 모두 있습니다:
  access("DEPARTMENT_ID"=80) — 인덱스 탐색 범위를 80으로 고정
  1번의 filter("DEPARTMENT_ID"=80) — 테이블 접근 후 재확인 (안전 장치)

BATCHED 방식: ROWID를 모아 블록 단위로 테이블을 읽어 Random I/O를 줄입니다.`,
...
reducing random I/O by reading table blocks more efficiently.`,
    callStatsAnnotations: [
      { line: 'Fetch  1  0.00  0.00  1  4  0  2', noteKo: 'Query=4 → 논리 블록 4개(인덱스 2 + 테이블 2). Disk=1 → 첫 실행이라 인덱스 블록 1개가 캐시에 없었습니다. BATCHED 방식으로 ROWID를 모아 처리했으므로 테이블 랜덤 I/O가 최소화됩니다.', noteEn: 'Query=4 → 4 logical blocks (2 index + 2 table). Disk=1 → one index block was not cached on first run. BATCHED mode collected ROWIDs before probing the table, minimising random table I/O.' },
    ],
    predicateAnnotations: [
      { line: '2 - access("DEPARTMENT_ID"=80)', noteKo: 'Op 2(INDEX RANGE SCAN)에서 DEPARTMENT_ID=80이 access — 인덱스의 탐색 시작점과 끝점을 모두 80으로 고정합니다. B-Tree 인덱스에서 DEPARTMENT_ID=80인 리프 블록 범위만 읽습니다.', noteEn: 'Op 2 (INDEX RANGE SCAN): DEPARTMENT_ID=80 is an access predicate — it pins both the start and end of the B-tree scan range to 80. Only leaf blocks containing DEPARTMENT_ID=80 entries are read.' },
      { line: '1 - filter("DEPARTMENT_ID"=80)', noteKo: 'Op 1(TABLE ACCESS)에서 동일 조건이 filter로 다시 나옵니다. 인덱스 스캔 후 테이블 행을 읽어올 때 Oracle이 한 번 더 값을 확인하는 안전 장치입니다. Unique 인덱스가 아닌 경우 이 패턴이 흔합니다.', noteEn: 'Op 1 (TABLE ACCESS) re-checks the same condition as a filter. After reading a table row via ROWID, Oracle verifies DEPARTMENT_ID=80 once more as a safety net. This double-check pattern is common with non-unique indexes.' },
    ],
    projectionAnnotations: [
      { line: '1 - output: [EMPLOYEE_ID, LAST_NAME, JOB_ID, SALARY]', noteKo: 'TABLE ACCESS 단계에서 SELECT 절에 필요한 4개 컬럼을 출력합니다. DEPARTMENT_ID는 이미 인덱스에서 확인했으므로 여기선 포함되지 않습니다.', noteEn: 'TABLE ACCESS projects the 4 columns required by SELECT. DEPARTMENT_ID was already verified by the index and is not passed up again.' },
    ...
    statsAnnotations: [
      { line: 'consistent gets      =  4', noteKo: '논리 블록 4개: 인덱스 2(루트→리프) + 테이블 2(배치된 2개 ROWID). Q1의 2블록보다 많지만 FTS의 8블록보다 훨씬 적습니다.', noteEn: '4 logical blocks: 2 index (root→leaf) + 2 table (batched ROWID lookups). More than Q1\'s 2 blocks, but far less than FTS\'s 8 blocks.' },
...
------- ------  ----  -------  ----  -----  -------  ----
total        3  0.00     0.00     0      4        0     1`,
    reasonKo: `WHERE employee_id = 103 은 정확히 1건만 반환하는 PK 조건입니다.
드라이빙 결과가 1건이므로 CBO는 Nested Loop Join을 선택합니다.
...
Hash Join 대신 Nested Loop를 선택한 이유:
  드라이빙 결과가 1건이므로 DEPARTMENTS를 1번만 탐색하면 됩니다.
  Hash Join은 해시 테이블 구성 오버헤드가 있어 소량 조인에 불리합니다.
...
Why Nested Loop instead of Hash Join?
  ...
  consistent gets = 4 (2 index + 2 table), physical reads = 0.`,
    predicateAnnotations: [
      { line: '3 - access("E"."EMPLOYEE_ID"=103)', noteKo: 'Op 3(INDEX UNIQUE SCAN, EMP_EMP_ID_PK)에서 EMPLOYEE_ID=103으로 PK 인덱스를 수직 탐색합니다. Unique 인덱스 + 등치 조건 → UNIQUE SCAN.', noteEn: 'Op 3 (INDEX UNIQUE SCAN on EMP_EMP_ID_PK): traverses the PK index vertically to find EMPLOYEE_ID=103. Unique index + equality = UNIQUE SCAN.' },
      { line: '5 - access("D"."DEPARTMENT_ID"="E"."DEPARTMENT_ID")', noteKo: 'Op 5(INDEX UNIQUE SCAN, DEPT_ID_PK)에서 Op 2가 꺼낸 DEPARTMENT_ID 값을 DEPARTMENTS PK로 탐색합니다. 이 값은 쿼리 파싱 시에는 모르고, 런타임에 Op 2 결과로부터 받습니다(NL의 핵심: 드라이빙 행마다 이너를 탐색).', noteEn: 'Op 5 (INDEX UNIQUE SCAN on DEPT_ID_PK): probes DEPARTMENTS PK with the DEPARTMENT_ID value fetched by Op 2. This value is not known at parse time — it is supplied at runtime from the driving row (the core of Nested Loop: probe inner once per driving row).' },
    ],
    projectionAnnotations: [
      { line: '2 - output: [E.EMPLOYEE_ID, E.LAST_NAME, E.SALARY, E.DEPARTMENT_ID]', noteKo: 'EMPLOYEES 테이블에서 4개 컬럼을 꺼냅니다. 이 중 DEPARTMENT_ID가 다음 단계(Op 5)의 조인 키로 전달됩니다.', noteEn: 'Fetches 4 columns from EMPLOYEES. DEPARTMENT_ID among them is passed to Op 5 as the join key for the next probe.' },
      { line: '4 - output: [D.DEPARTMENT_NAME]', noteKo: 'DEPARTMENTS 테이블에서 DEPARTMENT_NAME 하나만 꺼냅니다. 나머지 컬럼(MANAGER_ID, LOCATION_ID 등)은 SELECT 절에 없으므로 읽지 않습니다.', noteEn: 'Only DEPARTMENT_NAME is projected from DEPARTMENTS. Other columns (MANAGER_ID, LOCATION_ID, etc.) are not in the SELECT clause and are not read.' },
    ],
    statsAnnotations: [
      { line: 'consistent gets      =  4', noteKo: '논리 블록 4개: 인덱스 1(EMP PK) + 테이블 1(EMPLOYEES) + 인덱스 1(DEPT PK) + 테이블 1(DEPARTMENTS). 2개 테이블 조인 최솟값입니다.', noteEn: '4 logical blocks: 1 index (EMP PK) + 1 table (EMPLOYEES) + 1 index (DEPT PK) + 1 table (DEPARTMENTS). The minimum possible for a two-table join via PK.' },
      { line: 'sorts (memory)       =  0', noteKo: 'Nested Loop Join은 정렬이 필요 없으므로 sorts=0. Hash Join은 해시 테이블 구성, Sort Merge Join은 정렬 단계가 필요합니다.', noteEn: 'Nested Loop Join requires no sorting → sorts=0. Hash Join needs to build a hash table; Sort Merge Join needs explicit sort steps.' },
    ],
  },
...
------- ------  ----  -------  ----  -----  -------  ----
total        4  0.01     0.08     5     18        0    14`,
    reasonKo: `이 쿼리는 특정 행을 조건으로 추리는 것이 아니라 전체를 읽어 집계합니다.
인덱스로 범위를 줄일 수 없으므로 두 테이블 모두 TABLE ACCESS FULL을 선택합니다.

실행 순서: ④ → ⑤ → ③ → ② → ① → ⓪

Hash Join — Build Input 선택 이유:
  DEPARTMENTS(20행) < EMPLOYEES(15행)이지만 DEPARTMENTS의 BLOCKS가 더 적습니다.
  CBO는 더 작은 테이블을 Build Input(해시 테이블 구성)으로 씁니다 → ④ DEPARTMENTS.
  EMPLOYEES는 Probe Input으로 한 줄씩 읽으며 해시 테이블을 탐색합니다 → ⑤.

③ HASH JOIN: access("E"."DEPARTMENT_ID"="D"."DEPARTMENT_ID")
  조인 조건이 access로 표시됩니다 — Hash Join에서 조인 키는 항상 access.
...
Column Projection에서 ②의 출력에 AVG 대신 SUM + COUNT(*)가 있습니다.
  AVG는 나중에 SUM/COUNT로 계산하기 위해 내부적으로 분해됩니다.`,
...
Notice in Column Projection that op ② outputs SUM + COUNT(*) instead of AVG.
  Oracle decomposes AVG into SUM/COUNT internally and computes the final ratio at output time.`,
    predicateAnnotations: [
      { line: '3 - access("E"."DEPARTMENT_ID"="D"."DEPARTMENT_ID")', noteKo: 'Op 3(HASH JOIN)의 조인 키가 access입니다. Hash Join은 작은 테이블(DEPARTMENTS)로 해시 테이블을 만든 뒤, 큰 테이블(EMPLOYEES)의 각 행을 읽어 DEPARTMENT_ID 해시 값으로 버킷을 탐색합니다. 조인 키는 항상 access로 표시됩니다.', noteEn: 'Op 3 (HASH JOIN) join key shown as access. Hash Join builds a hash table from DEPARTMENTS, then probes each EMPLOYEES row by hashing DEPARTMENT_ID to find the matching bucket. Join keys in Hash Join are always labeled access.' },
    ],
    projectionAnnotations: [
      { line: '2 - output: [D.DEPARTMENT_NAME, COUNT(E.EMPLOYEE_ID), SUM(E.SALARY), COUNT(*), MAX(E.SALARY)]', noteKo: 'AVG(E.SALARY)가 SUM과 COUNT(*)로 분해되어 있습니다. Oracle은 집계 중간 상태를 SUM/COUNT로 유지하다가 최종 출력 시점에 나눗셈을 수행합니다. GROUP BY가 완료되기 전까지는 AVG를 직접 계산할 수 없기 때문입니다.', noteEn: 'AVG(E.SALARY) is decomposed into SUM + COUNT(*). Oracle maintains the running aggregate as a sum and count, then divides at the final projection step. A true average cannot be computed until all rows in a group have been seen.' },
      { line: '5 - output: [E.EMPLOYEE_ID, E.SALARY, E.DEPARTMENT_ID]', noteKo: 'EMPLOYEES FTS에서 집계·조인에 필요한 3개 컬럼만 추출합니다. LAST_NAME 등 SELECT에 없는 컬럼은 읽지 않습니다.', noteEn: 'EMPLOYEES FTS projects only the 3 columns needed for aggregation and the join key. Columns not in the SELECT clause (LAST_NAME, etc.) are not read.' },
    ],
    statsAnnotations: [
      { line: 'consistent gets      = 18', noteKo: '논리 블록 18개: DEPARTMENTS 8 + EMPLOYEES 10. 두 테이블 모두 FTS이므로 각 테이블 전체 블록을 읽은 값입니다.', noteEn: '18 logical blocks: 8 (DEPARTMENTS FTS) + 10 (EMPLOYEES FTS). Both tables are fully scanned, so CR equals the sum of each table\'s block count.' },
      { line: 'sorts (memory)       =  1', noteKo: 'ORDER BY avg_salary DESC로 인해 SORT ORDER BY 1회 발생. HASH GROUP BY는 정렬이 필요 없지만, 최종 결과를 정렬해야 하므로 메모리 정렬 1회가 발생합니다.', noteEn: 'ORDER BY avg_salary DESC triggers 1 in-memory sort. HASH GROUP BY itself needs no sort, but the final ORDER BY requires SORT ORDER BY to arrange the 14 grouped rows.' },
    ],
  },
...
------- ------  ----  -------  ----  -----  -------  ----
total        3  0.00     0.00     0      4        0     2`,
    reasonKo: `두 가지 조건이 있지만 Predicate Information을 보면 서로 다르게 처리됩니다.

  2번 access("E"."DEPARTMENT_ID"=80)
    EMP_DEPARTMENT_IX 인덱스의 선두 컬럼이 DEPARTMENT_ID입니다.
    인덱스가 이 조건으로 탐색 범위를 결정합니다 → access.
    인덱스만으로 DEPARTMENT_ID=80인 행을 빠르게 찾아 ROWID를 반환합니다.

  1번 filter("E"."SALARY">8000)
    EMP_DEPARTMENT_IX 인덱스에는 SALARY 컬럼이 없습니다.
    따라서 테이블을 먼저 읽고(TABLE ACCESS BY INDEX ROWID) 나서야
    SALARY 값을 확인할 수 있습니다 → filter.

핵심: 같은 쿼리의 두 조건이 access(인덱스 탐색)와 filter(테이블 후처리)로 나뉩니다.

최적화 힌트:
  (DEPARTMENT_ID, SALARY)로 복합 인덱스를 만들면
  SALARY > 8000도 access 범위로 처리할 수 있어 테이블 접근 횟수가 줄어듭니다.`,
...
Optimization hint:
  ...
  an access predicate too, reducing the number of table rows visited.`,
    predicateAnnotations: [
      { line: '2 - access("E"."DEPARTMENT_ID"=80)', noteKo: 'DEPARTMENT_ID=80이 인덱스 탐색 범위를 결정합니다. INDEX RANGE SCAN은 인덱스 B-Tree에서 이 범위의 리프 블록만 읽습니다. SALARY 조건은 이 시점에서 처리할 수 없습니다(인덱스에 없으므로).', noteEn: 'DEPARTMENT_ID=80 determines the index scan range. INDEX RANGE SCAN reads only the leaf blocks within this range. The SALARY condition cannot be applied here — it is not in the index.' },
      { line: '1 - filter("E"."SALARY">8000)', noteKo: '테이블 행을 ROWID로 읽어온 후에야 SALARY 컬럼 값을 확인할 수 있습니다. DEPARTMENT_ID=80인 행 중 SALARY≤8000인 행은 이 단계에서 버려집니다. 인덱스가 SALARY를 포함하지 않으므로 불가피한 테이블 접근입니다.', noteEn: 'SALARY can only be checked after reading the table row via ROWID. Rows with DEPARTMENT_ID=80 but SALARY≤8000 are discarded at this step. This table access is unavoidable because the index does not include SALARY.' },
    ],
    projectionAnnotations: [
      { line: '2 - output: [ROWID, E.DEPARTMENT_ID]', noteKo: 'INDEX RANGE SCAN은 ROWID와 인덱스 키(DEPARTMENT_ID)만 출력합니다. SALARY는 인덱스에 없으므로 여기서 나오지 않습니다. 만약 (DEPARTMENT_ID, SALARY) 복합 인덱스였다면 여기서 SALARY도 출력해 테이블 접근을 생략할 수 있습니다.', noteEn: 'INDEX RANGE SCAN outputs only ROWID and the index key DEPARTMENT_ID. SALARY is not in the index so it does not appear here. A composite index on (DEPARTMENT_ID, SALARY) would allow SALARY to be output here, potentially avoiding the table access entirely.' },
    ],
    statsAnnotations: [
      { line: 'consistent gets      =  4', noteKo: '인덱스 2블록 + 테이블 2블록. SALARY 필터 후 2행만 남았지만 인덱스에서 찾은 ROWID마다 테이블을 읽어야 했습니다.', noteEn: '2 index blocks + 2 table blocks. Only 2 rows passed the SALARY filter, but each ROWID from the index required one table block read.' },
    ],
  },
...
Column Projection에서 ③의 출력은 JH.EMPLOYEE_ID만 있습니다.
  EXISTS는 값이 아닌 존재 여부만 확인하므로 JOB_HISTORY 테이블 본체를 읽지 않습니다.
  인덱스만으로 처리 완료 → 효율적.`,
...
Column Projection shows op ③ outputs only JH.EMPLOYEE_ID.
  ...
  The table itself is never visited; the index alone is sufficient → very efficient.`,
    predicateAnnotations: [
      { line: '3 - access("JH"."EMPLOYEE_ID"="E"."EMPLOYEE_ID")', noteKo: 'Op 3(INDEX RANGE SCAN)에서 JOB_HISTORY PK 인덱스를 EMPLOYEE_ID로 탐색합니다. 이 값은 외부 루프(EMPLOYEES)에서 하나씩 공급됩니다. SEMI Join이므로 첫 번째 매칭 즉시 이 인덱스 탐색을 멈추고 다음 외부 행으로 진행합니다.', noteEn: 'Op 3 (INDEX RANGE SCAN) probes the JOB_HISTORY PK index with EMPLOYEE_ID supplied by the outer loop (EMPLOYEES). Because this is a SEMI join, the index scan stops at the first match and moves on to the next outer row immediately.' },
    ],
    projectionAnnotations: [
      { line: '3 - output: [JH.EMPLOYEE_ID]', noteKo: 'EXISTS 서브쿼리는 존재 여부만 확인하므로 JH.EMPLOYEE_ID 하나만 출력합니다. SELECT 1이기 때문에 JOB_HISTORY의 다른 컬럼(JOB_ID, START_DATE 등)은 전혀 읽지 않습니다. 심지어 JOB_HISTORY 테이블 본체도 접근하지 않습니다 — 인덱스만으로 충분합니다.', noteEn: 'EXISTS only needs to verify presence, so only JH.EMPLOYEE_ID is output. No other JOB_HISTORY columns (JOB_ID, START_DATE, etc.) are read. The JOB_HISTORY table itself is never accessed — the index alone provides the EMPLOYEE_ID needed for the existence check.' },
    ],
    statsAnnotations: [
      { line: 'consistent gets      = 14', noteKo: '논리 블록 14개: EMPLOYEES FTS 8 + JOB_HISTORY 인덱스 탐색 6. 매칭 즉시 중단(SEMI)하므로 15회를 모두 스캔하더라도 인덱스 블록 수는 최소화됩니다.', noteEn: '14 logical blocks: 8 (EMPLOYEES FTS) + 6 (JOB_HISTORY index probes). SEMI stop-on-first-match limits the index reads even across all 15 outer rows.' },
      { line: 'rows processed       =  5', noteKo: '최종 반환 5명. JOB_HISTORY에 이력이 있는 직원 수입니다. 나머지 10명은 EXISTS 조건을 통과하지 못해 제거됩니다.', noteEn: '5 employees returned — those who have at least one JOB_HISTORY record. The remaining 10 employees failed the EXISTS check and were filtered out.' },
    ],
  },
...
------- ------  ----  -------  ----  -----  -------  ----
total        3  0.01     0.12     8     28        0     7`,
    reasonKo: `3테이블 조인에서 CBO가 조인 순서를 결정하는 방법을 볼 수 있습니다.

실행 순서: ⑤ → ④ → ⑥ → ③ → ⑦ → ② → ① → ⓪

조인 순서 선택 이유:
  WHERE l.country_id = 'US' 조건으로 LOCATIONS가 가장 먼저 줄어듭니다.
  LOC_COUNTRY_IX 인덱스로 COUNTRY_ID='US'를 access한 뒤 (④⑤),
  4개 미국 위치를 Build Input으로 해시 테이블을 구성합니다.

  DEPARTMENTS(20행)를 Probe로 읽어 LOCATION_ID로 조인 → 미국 부서 4개만 남음 (③⑥).
  마지막으로 EMPLOYEES를 읽어 DEPARTMENT_ID로 조인 → 7명의 미국 근무 직원 (②⑦).

4번 Predicate: access와 filter가 동시에 있습니다.
...
ORDER BY e.last_name triggers ① SORT ORDER BY: sorts(memory)=1.
  7 rows sorted entirely in PGA memory — no disk spill.`,
    predicateAnnotations: [
      { line: '5 - access("L"."COUNTRY_ID"=\'US\')', noteKo: 'LOC_COUNTRY_IX 인덱스로 COUNTRY_ID=\'US\' 범위를 결정합니다. 22개 위치 중 미국 4개만 ROWID로 반환됩니다.', noteEn: 'LOC_COUNTRY_IX determines the scan range for COUNTRY_ID=\'US\'. Of 22 locations, only the 4 US ones are returned as ROWIDs.' },
      { line: '4 - filter("L"."COUNTRY_ID"=\'US\')', noteKo: '테이블 접근 후 같은 조건을 재확인합니다. 인덱스 탐색 결과와 테이블 데이터가 일치하는지 방어적으로 검사하는 Oracle의 패턴입니다.', noteEn: 'After reading the table row, the same predicate is re-checked as a defensive safety net. This double-check is a common Oracle pattern when using non-unique indexes.' },
      { line: '3 - access("D"."LOCATION_ID"="L"."LOCATION_ID")', noteKo: 'LOCATIONS 결과(미국 4개)로 해시 테이블을 구성한 뒤, DEPARTMENTS의 LOCATION_ID로 해시 버킷을 탐색합니다. Hash Join 조인 키 → access.', noteEn: 'Hash table built from LOCATIONS (4 US rows). DEPARTMENTS rows probe the hash table using LOCATION_ID. Hash Join join key → always access.' },
      { line: '2 - access("E"."DEPARTMENT_ID"="D"."DEPARTMENT_ID")', noteKo: '미국 부서 4개와 EMPLOYEES를 DEPARTMENT_ID로 Hash Join. 결과 7명의 미국 근무 직원이 추출됩니다.', noteEn: 'Hash Join between the 4 US departments and EMPLOYEES on DEPARTMENT_ID. Result: 7 employees working in US offices.' },
    ],
    statsAnnotations: [
      { line: 'consistent gets      = 28', noteKo: '논리 블록 28개: LOCATIONS 인덱스/테이블 8 + DEPARTMENTS FTS 8 + EMPLOYEES FTS 10 + SORT 2. 3테이블 조인의 누적 CR입니다.', noteEn: '28 logical blocks: ~8 (LOCATIONS index+table) + 8 (DEPARTMENTS FTS) + 10 (EMPLOYEES FTS) + 2 (sort overhead). Cumulative CR for a three-table join.' },
      { line: 'sorts (memory)       =  1', noteKo: 'ORDER BY e.last_name으로 최종 7행을 PGA 메모리에서 정렬. pw=0이므로 TEMP 스필은 없습니다.', noteEn: 'ORDER BY e.last_name sorts the final 7 rows in PGA memory. pw=0 confirms no TEMP spill.' },
    ],
  },
...
------- ------  ----  -------  ----  -----  -------  ----
total        4  0.00     0.05     3      8        0    14`,
    reasonKo: `RANK() OVER (PARTITION BY ... ORDER BY ...)는 윈도우 함수입니다.
윈도우 함수는 반드시 전체 데이터를 메모리에 올려 정렬 후 계산해야 합니다.
이 때문에 WINDOW SORT 오퍼레이션이 항상 등장합니다.

실행 순서: ② → ① → ⓪

② TABLE ACCESS FULL (EMPLOYEES):
  DEPARTMENT_ID IS NOT NULL 조건 — filter(인덱스 없음).
  NULL이 아닌 직원 14행을 반환합니다.
  DEPARTMENT_ID가 NULL인 직원(manager_id=null인 일부)을 제외합니다.
...
RANK vs DENSE_RANK:
  같은 급여가 있으면 RANK는 동순위 후 번호를 건너뜁니다 (1,1,3).
  DENSE_RANK는 건너뛰지 않습니다 (1,1,2).
  두 함수 모두 WINDOW SORT 오퍼레이션을 사용합니다.`,
...
RANK vs DENSE_RANK:
  ...
  Both functions use the WINDOW SORT operation.`,
    predicateAnnotations: [
      { line: '2 - filter("E"."DEPARTMENT_ID" IS NOT NULL)', noteKo: 'TABLE ACCESS FULL에서 DEPARTMENT_ID IS NOT NULL이 filter입니다. IS NOT NULL은 인덱스를 사용할 수 없는 조건 중 하나이며(NULL 값은 B-Tree 인덱스에 저장되지 않음), 전체 테이블을 읽은 후 NULL인 행을 제거합니다.', noteEn: 'DEPARTMENT_ID IS NOT NULL is a filter on the TABLE ACCESS FULL. IS NOT NULL cannot use a B-tree index (NULL values are not stored in B-tree indexes), so every row is read and then filtered.' },
    ],
    projectionAnnotations: [
      { line: '1 - output: [E.EMPLOYEE_ID, E.LAST_NAME, E.DEPARTMENT_ID, E.SALARY, RANK() OVER (...)]', noteKo: 'WINDOW SORT 완료 후 RANK() 계산 결과가 추가되어 최종 5개 컬럼을 출력합니다. RANK()는 이 시점(정렬 후)에만 계산 가능합니다.', noteEn: 'After WINDOW SORT completes, the RANK() value is computed and appended, producing 5 output columns. RANK() can only be calculated after all rows are sorted.' },
      { line: '2 - output: [E.EMPLOYEE_ID, E.LAST_NAME, E.DEPARTMENT_ID, E.SALARY]', noteKo: 'TABLE ACCESS FULL은 RANK() 없이 4개 컬럼만 출력합니다. RANK() 계산은 상위 WINDOW SORT 단계에서 수행됩니다.', noteEn: 'TABLE ACCESS FULL outputs 4 columns without RANK(). The rank computation happens at the parent WINDOW SORT step once all rows are sorted.' },
    ],
    statsAnnotations: [
      { line: 'sorts (memory)       =  1', noteKo: 'WINDOW SORT 1회, 메모리에서 처리. 14행은 충분히 작아 PGA(Work Area)에서 완전히 정렬됩니다. 수백만 행이면 sorts(disk)가 증가하고 TEMP 테이블스페이스에 스필이 발생합니다.', noteEn: '1 in-memory sort via WINDOW SORT. 14 rows fit comfortably in PGA work area. With millions of rows, sorts(disk) would increase and data would spill to the TEMP tablespace.' },
      { line: 'rows processed       = 14', noteKo: 'DEPARTMENT_ID가 NULL인 직원 1명을 제외한 14명이 반환됩니다.', noteEn: '14 employees returned — all except the 1 whose DEPARTMENT_ID is NULL.' },
    ],
  },
...
CBO는 이를 독립적으로 실행한 뒤 결과를 조인에 사용합니다.
...
EMPLOYEES가 두 번 스캔(⑤, ⑧)되는 이유:
  CBO는 인라인 뷰를 메인 쿼리에 머지(merge)하지 않았습니다.
  쿼리 변환(Query Transformation)의 View Merging이 적용되지 않은 케이스입니다.
  더 큰 테이블에서는 이 이중 스캔이 병목이 될 수 있습니다.`,
...
Why EMPLOYEES is scanned twice (⑤ and ⑧):
  ...
  a WITH clause (CTE) or materialized hint can avoid it.`,
    predicateAnnotations: [
      { line: '2 - access("AVG_INFO"."DEPARTMENT_ID"="E"."DEPARTMENT_ID")', noteKo: 'Op 2(최종 HASH JOIN)의 조인 키입니다. avg_info(9행)로 해시 테이블을 구성하고, 외부 EMPLOYEES 결과(15행)의 DEPARTMENT_ID로 해시 버킷을 탐색합니다.', noteEn: 'Join key for Op 2 (final HASH JOIN). avg_info (9 rows) builds the hash table; the outer EMPLOYEES result (15 rows) probes using DEPARTMENT_ID.' },
      { line: 'filter("E"."SALARY">="AVG_INFO"."AVG_SAL")', noteKo: '조인 후 추가 필터입니다. DEPARTMENT_ID가 같은 쌍 중 SALARY가 부서 평균 미만인 직원을 제거합니다. 조인 키와 다른 컬럼 비교이므로 access가 아닌 filter입니다.', noteEn: 'Post-join filter applied after matching by department_id. Removes employees whose salary is below their department average. This is a filter (not access) because it compares a different column than the join key.' },
      { line: '6 - access("E"."DEPARTMENT_ID"="D"."DEPARTMENT_ID")', noteKo: 'Op 6(내부 HASH JOIN): DEPARTMENTS와 EMPLOYEES를 DEPARTMENT_ID로 조인합니다. 이 조인은 메인 쿼리의 직원정보 + 부서명 결합입니다.', noteEn: 'Op 6 (inner HASH JOIN): joins DEPARTMENTS and EMPLOYEES on DEPARTMENT_ID. This is the main query\'s employee-info + department-name combination.' },
    ],
    projectionAnnotations: [
      { line: '3 - output: [AVG_INFO.DEPARTMENT_ID, AVG_INFO.AVG_SAL]', noteKo: 'VIEW(인라인 뷰 구체화 결과)는 DEPARTMENT_ID와 AVG_SAL 두 컬럼만 출력합니다. GROUP BY 집계 결과입니다.', noteEn: 'VIEW (materialized inline view) outputs only DEPARTMENT_ID and AVG_SAL — the two columns produced by the GROUP BY aggregation.' },
      { line: '5 - output: [SALARY, DEPARTMENT_ID]', noteKo: '인라인 뷰 내부 EMPLOYEES FTS는 AVG 계산에 필요한 SALARY와 GROUP BY 키인 DEPARTMENT_ID만 꺼냅니다. EMPLOYEE_ID, LAST_NAME 등은 이 단계에서 필요하지 않습니다.', noteEn: 'The inline view\'s internal EMPLOYEES FTS projects only SALARY (for AVG) and DEPARTMENT_ID (for GROUP BY). EMPLOYEE_ID, LAST_NAME, etc. are not needed at this stage.' },
    ],
    statsAnnotations: [
      { line: 'consistent gets      = 30', noteKo: '논리 블록 30개: 인라인 뷰 EMPLOYEES FTS 8 + DEPARTMENTS FTS 8 + 메인쿼리 EMPLOYEES FTS 8 + LOCATIONS 6. EMPLOYEES 이중 스캔이 CR을 높입니다.', noteEn: '30 logical blocks: inline view EMPLOYEES FTS (8) + DEPARTMENTS FTS (8) + main query EMPLOYEES FTS (8) + overhead (6). The double EMPLOYEES scan inflates the CR.' },
      { line: 'rows processed       =  8', noteKo: '8명이 자신의 부서 평균 급여 이상을 받는 직원입니다. 나머지 7명은 filter("SALARY">="AVG_SAL")에서 제거됩니다.', noteEn: '8 employees earn at or above their department\'s average salary. The remaining 7 are removed by the filter("SALARY">="AVG_SAL") step.' },
    ],
  },
]
