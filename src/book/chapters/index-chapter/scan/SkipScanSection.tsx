...
const T = {
  ko: {
    ...
    subtitle: '복합 인덱스에서 선두(맨 앞) 컬럼이 WHERE 절에 없어도 인덱스를 활용할 수 있는 스캔 방식이에요. 선두 컬럼의 고유값마다 내부적으로 Range Scan을 한 번씩 수행해요.',
    whatTitle: '언제 사용되나요?',
    whatDesc: "Index Skip Scan은 복합 인덱스의 논리적 하위 인덱스(logical subindexes)를 활용해요. 마치 별도의 인덱스를 여러 개 검색하는 것처럼 하나의 인덱스 안을 '건너뛰며(skip)' 읽는 거예요. 예를 들어 복합 인덱스 (GENDER, SALARY)에서 GENDER 없이 SALARY만으로 검색할 때처럼, 선두 컬럼의 Cardinality(고유값 수)가 아주 낮을 때 옵티마이저(Optimizer, 실행 계획을 짜는 오라클의 두뇌)가 이 방식을 골라요. 선두 컬럼의 고유값이 적을수록 내부 Range Scan 횟수가 줄어서 효율적이에요.",
    howTitle: '어떻게 동작하나요?',
    step1Title: '① 선두 컬럼의 고유값 파악',
    step1Desc: "옵티마이저가 통계에서 선두 컬럼(GENDER)의 고유값 목록을 파악해요. 예를 들면 F, M 이런 식으로요. 이 값의 개수가 내부에서 몇 번 Range Scan을 할지 결정해요.",
    step2Title: '② 고유값마다 Range Scan 수행',
    step2Desc: "GENDER='F'인 구간에서 SALARY 조건으로 Range Scan을 하고, 그다음 GENDER='M' 구간에서 다시 Range Scan을 해요. 각 구간에서 조건을 만족하지 않는 항목은 건너뛰고요. 모든 구간의 결과를 합쳐서 돌려줘요.",
    sqlLabel: 'SQL 조건',
    ...
    traitTitle: '특징',
    traits: [
      { icon: '🎯', title: '선두 컬럼 없이도 인덱스를 써요', desc: '선두 컬럼이 WHERE에 없어도 인덱스를 탈 수 있어서 테이블 전체를 읽는 Full Table Scan을 피할 수 있어요.' },
      { icon: '📉', title: '선두 컬럼 고유값이 적을 때 제일 좋아요', desc: "GENDER처럼 'M'/'F' 두 값만 있으면 내부 Range Scan이 딱 2번뿐이에요. 고유값이 많아질수록 오히려 비효율적이에요." },
      { icon: '⚠️', title: '고유값이 많으면 비효율이에요', desc: '선두 컬럼 고유값이 100개라면 내부 Range Scan도 100번이나 수행돼요. 이 경우에는 Full Table Scan이 더 나을 수 있어요.' },
    ],
    cardinalityNote: 'Skip Scan의 효율은 선두 컬럼의 Cardinality(고유값 수)에 반비례해요. 고유값이 2~3개(성별, 상태 코드 등)일 때 가장 효과적이고, 고유값이 수백 개 이상이면 옵티마이저가 Skip Scan 대신 Full Table Scan을 선택하는 경우가 많아요.',
    hintTitle: 'Index Skip Scan이 쓰이는 상황',
    hintNote: '선두 컬럼이 WHERE 절에 없어도, 그 컬럼의 고유값 수(Cardinality)가 아주 적을 때 옵티마이저가 자동으로 선택해요. INDEX_SS 힌트로 강제할 수도 있어요.',
    examples: [
      { badge: '선두 컬럼 없는 검색', badgeColor: 'violet' as const,
        desc: "복합 인덱스 (gender, salary)에서 gender 없이 salary만 검색할 때, 오라클은 통계에서 gender의 고유값(M, F)을 찾아내서 'gender=F이면서 salary≥6000'과 'gender=M이면서 salary≥6000'을 각각 Range Scan해요. 내부적으로 2번의 Range Scan만으로 전체 결과를 얻을 수 있어요.",
      ...
      { badge: '상태 코드 + 날짜', badgeColor: 'violet' as const,
        desc: "상태 코드(ACTIVE/INACTIVE)처럼 고유값이 2개인 컬럼이 선두인 복합 인덱스에서 두 번째 컬럼만 검색하면 내부 Range Scan이 딱 2번만 발생해요. 이런 경우에는 테이블 전체를 읽는 것보다 훨씬 효율적이에요.",
      ...
      { badge: 'INDEX_SS 힌트', badgeColor: 'blue' as const,
        desc: '옵티마이저가 통계가 없거나 선택도 추정을 잘못해서 Full Table Scan을 선택했을 때 Skip Scan을 강제하는 방법이에요. 선두 컬럼 고유값이 적고 뒤쪽 컬럼 조건이 좁게 걸릴 때 효과적이에요.',
        sql: `SELECT /*+ INDEX_SS(e IDX_EMP_GENDER_SAL) */ *\nFROM employees e\nWHERE salary BETWEEN 5000 AND 8000;\n-- 옵티마이저가 Full Table Scan 선택했을 때\n-- Skip Scan 강제 — gender 고유값이 적을 때 유효` },
      { badge: '⚠️ Skip Scan이 비효율인 케이스', badgeColor: 'amber' as const,
        desc: '선두 컬럼 고유값이 많을수록 내부 Range Scan 횟수도 그만큼 늘어나요. department_id 고유값이 27개라면 Range Scan을 27번이나 수행하게 돼요. 이렇게 되면 인덱스 탐색 오버헤드가 오히려 Full Table Scan보다 느려질 수 있어요.',
  ...
  en: {
    ...
    whatDesc: "An index skip scan uses logical subindexes of a composite index. The database 'skips' through a single index as if it were searching separate indexes. The optimizer chooses Skip Scan when the leading column's cardinality (number of distinct values) is very low — for example, searching only on SALARY in a (GENDER, SALARY) composite index. The fewer distinct values the leading column has, the fewer internal Range Scans are needed.",
