...
export function ExplainPlanTable({
  ...
  // ── 텍스트 색상 (밝은 배경 기준) ────────────────────────────────
  const C = {
    sep:     'var(--color-ink-3)', // gray-400
    header:  'var(--color-ink)', // gray-700
    pipe:    'var(--color-ink-2)', // gray-500
    id:      'var(--color-ink)',
    idPred:  'var(--color-amber)', // amber-600 — * 있는 행
    op:      'var(--color-ink)', // gray-900
    name:    'var(--color-ink)',
    rows:    'var(--color-ink)',
    cost:    'var(--color-ink)',
    time:    'var(--color-ink-2)',
    stat:    'var(--color-ink)',
  }
  ...
  return (
    <div className="my-4">
      {caption && (
        <p className="mb-1.5 font-mono text-[10px] font-bold uppercase tracking-widest text-ink-2">{caption}</p>
      )}
      <div className="overflow-x-auto rounded-panel border border-line bg-paper px-4 py-3">
      ...
      {hasNotes && (() => {
        ...
        const isKo = lang === 'ko'
        return (
          <div className="mt-4 overflow-hidden rounded-panel border border-line bg-paper-sunk">
            <div className="grid grid-cols-[3rem_5rem_1fr] border-b border-line bg-paper-sunk">
              <span className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-ink-2">Id</span>
              <span className="px-2 py-2 text-[10px] font-bold uppercase tracking-wider text-ink-2">{isKo ? '실행 순서' : 'Exec Order'}</span>
              <span className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-ink-2">{isKo ? '설명' : 'Description'}</span>
            </div>
            <div className="divide-y divide-line">
              {noteRows.map((row, idx) => (
                <div key={row.id} className="grid grid-cols-[3rem_5rem_1fr]">
                  <span className="px-3 py-2.5 font-mono text-xs font-bold text-amber self-start">{row.id}</span>
                  <span className="px-2 py-2.5 font-mono text-[11px] font-semibold text-ink self-start">
                  ...
                  <span className="px-3 py-2.5 text-xs leading-relaxed text-ink/70">{row.note}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {stages.map((s, i) => {
          ...
          return (
            <g key={i}>
              ...
              {/* Sub items */}
              {s.subs.map((sub, si) => (
                <text key={si} x={cx} y={SUBS_Y0 + si * 16} textAnchor="middle" fontSize={10} fill="var(--color-ink-2)">{sub}</text>
              ))}
            </g>
          )
        })}
      ...
      <svg viewBox={`0 0 ${SVG_W_CALC} ${SVG_H}`} width={SVG_W_CALC} height={SVG_H} className="mx-auto block" style={{ maxWidth: '100%' }}>
        <defs>
          <marker id="cbo-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
            <polygon points="0 0, 7 3.5, 0 7" fill="var(--color-ink-3)" />
          </marker>
          <marker id="cbo-arrow-blue" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
            <polygon points="0 0, 7 3.5, 0 7" fill="var(--color-blue)" />
          </marker>
        </defs>

        {/* Input */}
        <rect x={INPUT_X} y={INPUT_Y} width={INPUT_W} height={INPUT_H} rx={8} fill="var(--color-rail)" stroke="var(--color-blue)" strokeWidth={1.5} />
        <text x={INPUT_X + INPUT_W / 2} y={INPUT_Y + 22} textAnchor="middle" fontSize={11} fontWeight="600" fill="var(--color-blue)">{isKo ? '파싱된' : 'Parsed'}</text>
        <text x={INPUT_X + INPUT_W / 2} y={INPUT_Y + 37} textAnchor="middle" fontSize={11} fontWeight="600" fill="var(--color-blue)">{isKo ? '쿼리' : 'Query'}</text>
        <line x1={INPUT_X + INPUT_W} y1={INPUT_Y + INPUT_H / 2} x2={BOX1_X - 4} y2={INPUT_Y + INPUT_H / 2} stroke="var(--color-ink-3)" strokeWidth={1.5} markerEnd="url(#cbo-arrow)" />

        {/* BOX1: Query Transformer */}
        <rect x={BOX1_X} y={BOX_Y} width={BOX_W} height={BOX_H} rx={10} fill="var(--color-rail)" stroke="var(--color-blue)" strokeWidth={1.5} />
        <rect x={BOX1_X} y={BOX_Y} width={BOX_W} height={32} rx={10} fill="var(--color-blue)" />
        <rect x={BOX1_X} y={BOX_Y + 22} width={BOX_W} height={10} fill="var(--color-blue)" />
        ...
        {[isKo ? '서브쿼리 Unnesting' : 'Subquery Unnesting', isKo ? '뷰 Merging' : 'View Merging', isKo ? 'Predicate Pushdown' : 'Predicate Pushdown', isKo ? '쿼리 재작성' : 'Query Rewriting'].map((txt, i) => (
          <text key={i} x={BOX1_X + BOX_W / 2} y={BOX_Y + 54 + i * 22} textAnchor="middle" fontSize={10} fill="var(--color-blue)">{txt}</text>
        ))}
        <line x1={BOX1_X + BOX_W} y1={BOX_Y + BOX_H / 2} x2={BOX2_X - 4} y2={BOX_Y + BOX_H / 2} stroke="var(--color-ink-3)" strokeWidth={1.5} markerEnd="url(#cbo-arrow)" />

        {/* BOX2: Estimator */}
        <rect x={BOX2_X} y={BOX_Y} width={BOX_W} height={BOX_H} rx={10} fill="var(--color-paper-sunk)" stroke="var(--color-purple)" strokeWidth={1.5} />
        <rect x={BOX2_X} y={BOX_Y} width={BOX_W} height={32} rx={10} fill="var(--color-purple)" />
        <rect x={BOX2_X} y={BOX_Y + 22} width={BOX_W} height={10} fill="var(--color-purple)" />
        ...
        {[isKo ? '선택도(Selectivity)' : 'Selectivity', isKo ? '카디널리티(Cardinality)' : 'Cardinality', isKo ? '비용(Cost)' : 'Cost', isKo ? '→ 최소 비용 선택' : '→ Pick lowest cost'].map((txt, i) => (
          <text key={i} x={BOX2_X + BOX_W / 2} y={BOX_Y + 54 + i * 22} textAnchor="middle" fontSize={10} fill="var(--color-purple)">{txt}</text>
        ))}

        {/* Stats box */}
        <rect x={STATS_X} y={STATS_Y} width={STATS_W} height={STATS_H} rx={6} fill="var(--color-rail)" stroke="var(--color-amber)" strokeWidth={1.5} />
        <text x={STATS_X + STATS_W / 2} y={STATS_Y + 16} textAnchor="middle" fontSize={10} fontWeight="600" fill="var(--color-amber)">Optimizer Statistics</text>
        <text x={STATS_X + STATS_W / 2} y={STATS_Y + 30} textAnchor="middle" fontSize={9} fill="var(--color-amber)">{isKo ? 'DBMS_STATS / 딕셔너리' : 'DBMS_STATS / dictionary'}</text>
        <line x1={STATS_X + STATS_W / 2} y1={STATS_Y} x2={BOX2_X + BOX_W / 2} y2={BOX_Y + BOX_H + 4} stroke="var(--color-blue)" strokeWidth={1.5} strokeDasharray="4 3" markerEnd="url(#cbo-arrow-blue)" />
        <line x1={BOX2_X + BOX_W} y1={BOX_Y + BOX_H / 2} x2={BOX3_X - 4} y2={BOX_Y + BOX_H / 2} stroke="var(--color-ink-3)" strokeWidth={1.5} markerEnd="url(#cbo-arrow)" />

        {/* BOX3: Plan Generator */}
        <rect x={BOX3_X} y={BOX_Y} width={BOX_W} height={BOX_H} rx={10} fill="var(--color-rail)" stroke="var(--color-amber)" strokeWidth={1.5} />
        <rect x={BOX3_X} y={BOX_Y} width={BOX_W} height={32} rx={10} fill="var(--color-amber)" />
        <rect x={BOX3_X} y={BOX_Y + 22} width={BOX_W} height={10} fill="var(--color-amber)" />
        ...
        {[isKo ? '액세스 패스 탐색' : 'Access paths', isKo ? '조인 순서 탐색' : 'Join orders', isKo ? '조인 방법 탐색' : 'Join methods', isKo ? '최저 비용 계획 선택' : 'Select min-cost plan'].map((txt, i) => (
          <text key={i} x={BOX3_X + BOX_W / 2} y={BOX_Y + 54 + i * 22} textAnchor="middle" fontSize={10} fill="var(--color-red)">{txt}</text>
        ))}
        <line x1={BOX3_X + BOX_W} y1={BOX_Y + BOX_H / 2} x2={OUTPUT_X - 4} y2={OUTPUT_Y + OUTPUT_H / 2} stroke="var(--color-ink-3)" strokeWidth={1.5} markerEnd="url(#cbo-arrow)" />

        {/* Output */}
        <rect x={OUTPUT_X} y={OUTPUT_Y} width={OUTPUT_W} height={OUTPUT_H} rx={8} fill="var(--color-rail)" stroke="var(--color-green)" strokeWidth={1.5} />
        <text x={OUTPUT_X + OUTPUT_W / 2} y={OUTPUT_Y + 22} textAnchor="middle" fontSize={11} fontWeight="600" fill="var(--color-green)">{isKo ? '실행 계획' : 'Query Plan'}</text>
        <text x={OUTPUT_X + OUTPUT_W / 2} y={OUTPUT_Y + 38} textAnchor="middle" fontSize={10} fill="var(--color-green)">{isKo ? '(최저 비용)' : '(lowest cost)'}</text>
      </svg>
    </div>
  )
}
...
export function AccessPathDiagram({ mode, lang }: { mode: AccessMode; lang: 'ko' | 'en' }) {
  ...
  const blockFills = Array.from({ length: BLOCKS }, (_, bi) => {
    if (mode === 'fts') return 'var(--color-green)'
    if (mode === 'unique') return bi === 2 ? 'var(--color-amber)' : 'var(--color-rail)'
    if (mode === 'range') return bi >= 1 && bi <= 3 ? 'var(--color-blue)' : 'var(--color-rail)'
    return 'var(--color-rail)'
  })

  const leafFills = Array.from({ length: LEAVES }, (_, li) => {
    if (mode === 'fts') return 'var(--color-rail)'
    if (mode === 'unique') return li === 1 ? 'var(--color-amber)' : 'var(--color-rail)'
    if (mode === 'range') return li >= 0 && li <= 2 ? 'var(--color-blue)' : 'var(--color-rail)'
    return 'var(--color-rail)'
  })

  const legendItems =
    mode === 'fts'
      ? [{ color: 'var(--color-green)', label: isKo ? '읽는 블록 (전체)' : 'Read blocks (all)' }]
      : mode === 'unique'
        ? [{ color: 'var(--color-amber)', label: isKo ? '매칭 리프/블록 (1개)' : 'Matched leaf/block (1)' }, { color: 'var(--color-rail)', label: isKo ? '스캔 안 함' : 'Not scanned' }]
        : [{ color: 'var(--color-blue)', label: isKo ? '범위 내 리프/블록' : 'In-range leaf/blocks' }, { color: 'var(--color-rail)', label: isKo ? '범위 외' : 'Out of range' }]
  ...
  return (
    <div className="rounded-panel border border-line bg-paper-sunk p-3 my-4">
      <p className="text-center text-xs font-semibold text-ink mb-2">{title}</p>
      <div className="overflow-x-auto">
        <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} width={SVG_W} height={SVG_H} className="mx-auto block" style={{ maxWidth: '100%' }}>
          ...
          {mode !== 'fts' && (
            <>
              <rect x={rootCx - 36} y={IDX_Y} width={72} height={26} rx={6} fill="var(--color-rail)" stroke="var(--color-blue)" strokeWidth={1.5} />
              <text x={rootCx} y={IDX_Y + 17} textAnchor="middle" fontSize={11} fontWeight="600" fill="var(--color-blue)">Root</text>
              <rect x={rootCx - 30} y={IDX_Y + 42} width={60} height={24} rx={5} fill="var(--color-rail)" stroke="var(--color-blue)" strokeWidth={1.5} />
              <text x={rootCx} y={IDX_Y + 58} textAnchor="middle" fontSize={10} fill="var(--color-blue)">Branch</text>
              <line x1={rootCx} y1={IDX_Y + 26} x2={rootCx} y2={IDX_Y + 42} stroke="var(--color-blue)" strokeWidth={1.5} />
              {Array.from({ length: LEAVES }, (_, li) => {
                const lx = LEAF_START + li * (LEAF_W + LEAF_GAP)
                const cx = lx + LEAF_W / 2
                return (
                  <g key={li}>
                    <line x1={rootCx} y1={IDX_Y + 66} x2={cx} y2={LEAF_Y} stroke="var(--color-blue)" strokeWidth={1} strokeDasharray="3 2" />
                    <rect x={lx} y={LEAF_Y} width={LEAF_W} height={LEAF_H} rx={4} fill={leafFills[li]} stroke="var(--color-blue)" strokeWidth={1.5} />
                    <text x={cx} y={LEAF_Y + 18} textAnchor="middle" fontSize={9} fontWeight="600" fill="var(--color-blue)">L{li + 1}</text>
                  </g>
                )
              })}
              {Array.from({ length: LEAVES - 1 }, (_, li) => {
                ...
                return <line key={li} x1={lx1} y1={LEAF_Y + LEAF_H / 2} x2={lx2} y2={LEAF_Y + LEAF_H / 2} stroke="var(--color-blue)" strokeWidth={1} strokeDasharray="2 2" />
              })}
              <text x={IDX_X + IDX_W / 2} y={SVG_H - 6} textAnchor="middle" fontSize={10} fill="var(--color-ink-2)">{isKo ? 'B-Tree 인덱스' : 'B-Tree Index'}</text>
              {mode === 'unique' && (
                <line x1={LEAF_START + 1 * (LEAF_W + LEAF_GAP) + LEAF_W / 2} y1={LEAF_Y + LEAF_H} x2={TBL_X} y2={TBL_Y + 2 * (BLOCK_H + BLOCK_GAP) + BLOCK_H / 2} stroke="var(--color-blue)" strokeWidth={1.5} strokeDasharray="4 3" markerEnd="url(#ap-arrow)" />
              )}
              {mode === 'range' && (
                <>
                  <line x1={LEAF_START + 1 * (LEAF_W + LEAF_GAP) + LEAF_W} y1={LEAF_Y + LEAF_H / 2} x2={TBL_X} y2={TBL_Y + 1 * (BLOCK_H + BLOCK_GAP) + BLOCK_H / 2} stroke="var(--color-blue)" strokeWidth={1.5} strokeDasharray="4 3" markerEnd="url(#ap-arrow)" />
                  <line x1={LEAF_START + 2 * (LEAF_W + LEAF_GAP) + LEAF_W} y1={LEAF_Y + LEAF_H / 2} x2={TBL_X} y2={TBL_Y + 3 * (BLOCK_H + BLOCK_GAP) + BLOCK_H / 2} stroke="var(--color-blue)" strokeWidth={1.5} strokeDasharray="4 3" markerEnd="url(#ap-arrow)" />
                </>
              )}
            </>
          )}
          ...
          {mode === 'fts' && (
            <>
              <text x={80} y={SVG_H / 2 - 8} textAnchor="middle" fontSize={11} fontWeight="600" fill="var(--color-green)">{isKo ? '순차 읽기' : 'Sequential'}</text>
              <text x={80} y={SVG_H / 2 + 8} textAnchor="middle" fontSize={11} fontWeight="600" fill="var(--color-green)">{isKo ? '멀티블록 I/O' : 'Multi-block I/O'}</text>
              <line x1={150} y1={SVG_H / 2} x2={TBL_X - 6} y2={SVG_H / 2} stroke="var(--color-green)" strokeWidth={2} markerEnd="url(#ap-arrow)" />
            </>
          )}

          <text x={TBL_X + TBL_W / 2} y={TBL_Y - 6} textAnchor="middle" fontSize={11} fontWeight="600" fill="var(--color-ink-2)">{isKo ? '테이블 블록' : 'Table Blocks'}</text>
          {Array.from({ length: BLOCKS }, (_, bi) => {
            const by = TBL_Y + bi * (BLOCK_H + BLOCK_GAP)
            return (
              <g key={bi}>
                <rect x={TBL_X} y={by} width={TBL_W} height={BLOCK_H} rx={4} fill={blockFills[bi]} stroke="var(--color-line)" strokeWidth={1} />
                <text x={TBL_X + 10} y={by + 18} fontSize={9} fill="var(--color-ink-2)">Block #{bi + 1}</text>
              </g>
            )
          })}
          ...
          {legendItems.map((item, li) => (
            <g key={li}>
              <rect x={10} y={SVG_H - 30 + li * 16} width={12} height={12} rx={2} fill={item.color} stroke="var(--color-ink-3)" strokeWidth={1} />
              <text x={26} y={SVG_H - 20 + li * 16} fontSize={9} fill="var(--color-ink-2)">{item.label}</text>
            </g>
          ))}
        </svg>
      </div>
...
export function CostCompareChart({ lang }: { lang: 'ko' | 'en' }) {
  const isKo = lang === 'ko'
  const bars = [
    { label: isKo ? '전체 테이블 스캔\n(선택도 50%)' : 'Full Table Scan\n(50% selectivity)', cost: 90, color: 'var(--color-red)', textColor: 'var(--color-red)' },
    { label: isKo ? '전체 테이블 스캔\n(선택도 5%)' : 'Full Table Scan\n(5% selectivity)', cost: 90, color: 'var(--color-amber)', textColor: 'var(--color-red)' },
    { label: isKo ? 'Index Range Scan\n(선택도 5%)' : 'Index Range Scan\n(5% selectivity)', cost: 28, color: 'var(--color-green)', textColor: 'var(--color-green)' },
    { label: isKo ? 'Index Unique Scan\n(= 조건)' : 'Index Unique Scan\n(= predicate)', cost: 4, color: 'var(--color-green)', textColor: 'var(--color-green)' },
  ]
      ...
      <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} width={SVG_W} height={SVG_H} className="mx-auto block" style={{ maxWidth: '100%' }}>
        {bars.map((bar, i) => {
          ...
          return (
            <g key={i}>
              {bar.label.split('\n').map((line, li) => (
                <text key={li} x={LABEL_W - 8} y={y + 14 + li * 14} textAnchor="end" fontSize={10} fill="var(--color-ink-2)">{line}</text>
              ))}
              <rect x={LABEL_W} y={y + 4} width={MAX_BAR_W} height={BAR_H - 8} rx={4} fill="var(--color-rail)" />
        ...
        <text x={LABEL_W} y={SVG_H - 2} fontSize={9} fill="var(--color-ink-3)">
...
export function SelectivityWidget({ lang }: { lang: 'ko' | 'en' }) {
  ...
  const indexColor = recFts ? 'var(--color-red)' : 'var(--color-green)'
  const indexLabel = recFts
    ? isKo ? 'Full Table Scan 권장' : 'Full Table Scan preferred'
    : isKo ? 'Index Scan 권장' : 'Index Scan preferred'

  return (
    <div className="my-4 rounded-panel border border-purple/30 bg-purple/5 p-4">
      <p className="text-sm font-semibold text-purple mb-3">
        {isKo ? '선택도 계산기 (등치 조건: col = val)' : 'Selectivity Calculator (equality: col = val)'}
      </p>
      <div className="flex flex-wrap items-center gap-4">
        <div>
          <label className="text-xs text-purple font-medium block mb-1">
            NDV (NUM_DISTINCT): <span className="font-bold">{ndv}</span>
          </label>
          <input type="range" min={2} max={500} value={ndv} onChange={(e) => setNdv(Number(e.target.value))} className="w-40 accent-purple" />
        </div>
        <div className="flex gap-4 flex-wrap">
          <div className="rounded-card bg-paper border border-purple/30 px-4 py-2 text-center">
            <p className="text-[10px] text-ink-2">{isKo ? '선택도' : 'Selectivity'}</p>
            <p className="font-mono text-sm font-bold text-purple">{selectivity}</p>
            <p className="text-[10px] text-ink-2">1 / {ndv}</p>
          </div>
          <div className="rounded-card bg-paper border border-purple/30 px-4 py-2 text-center">
            <p className="text-[10px] text-ink-2">{isKo ? '예상 반환 행' : 'Est. Rows'}</p>
            <p className="font-mono text-sm font-bold text-purple">{cardinality.toLocaleString()}</p>
            <p className="text-[10px] text-ink-2">{numRows.toLocaleString()} × {selectivity}</p>
          </div>
          <div className="rounded-card border px-4 py-2 text-center" style={{ borderColor: indexColor, background: `${indexColor}22` }}>
            <p className="text-[10px] text-ink-2">{isKo ? 'CBO 권장' : 'CBO Preference'}</p>
            <p className="text-xs font-bold mt-1" style={{ color: recFts ? 'var(--color-red)' : 'var(--color-green)' }}>{indexLabel}</p>
          </div>
        </div>
      </div>
    </div>
...
export function PlanTreeDiagram({ lang }: { lang: 'ko' | 'en' }) {
  ...
  const nodes: Node[] = [
    { id: 0, label: 'SELECT STATEMENT', sub: isKo ? '최종 결과 반환' : 'Return result', color: 'var(--color-blue)', x: 250, y: 20 },
    { id: 1, label: 'HASH JOIN', sub: isKo ? '두 결과 집합 조인' : 'Join two row sets', color: 'var(--color-purple)', x: 250, y: 90, parentId: 0 },
    { id: 2, label: 'TABLE ACCESS FULL', sub: 'DEPARTMENTS', color: 'var(--color-amber)', x: 100, y: 170, parentId: 1 },
    { id: 3, label: 'INDEX RANGE SCAN', sub: 'EMP_DEPT_IX', color: 'var(--color-green)', x: 390, y: 170, parentId: 1 },
  ]
    ...
    <div className="my-4 overflow-x-auto">
      <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} width={SVG_W} height={SVG_H} className="mx-auto block" style={{ maxWidth: '100%' }}>
        ...
        {nodes.filter(n => n.parentId !== undefined).map(n => {
          ...
          return <line key={n.id} x1={parent.x + NODE_W / 2} y1={parent.y + NODE_H} x2={n.x + NODE_W / 2} y2={n.y} stroke="var(--color-ink-3)" strokeWidth={1.5} markerEnd="url(#pt-arrow)" />
        })}
        {nodes.map(n => (
          <g key={n.id}>
            ...
            {n.sub && <text x={n.x + NODE_W / 2} y={n.y + 32} textAnchor="middle" fontSize={10} fill="var(--color-ink-2)">{n.sub}</text>}
          </g>
        ))}
        <text x={SVG_W / 2} y={SVG_H - 8} textAnchor="middle" fontSize={10} fill="var(--color-ink-3)">
...
export function SqlTraceDisplay({ lang }: { lang: 'ko' | 'en' }) {
  ...
  const pipeColor = 'var(--color-ink-2)'
  const labelColor = 'var(--color-ink)'
  const totalColor = 'var(--color-ink)'
  const phaseColors: Record<string, string> = {
    Parse:   'var(--color-ink)',
    Execute: 'var(--color-ink)',
    Fetch:   'var(--color-ink)',
  }
  ...
  return (
    <div className="my-4">
      {caption && (
        <p className="mb-1 text-xs font-semibold text-ink-2">{caption}</p>
      )}
      <pre className="overflow-x-auto rounded-card bg-paper border border-line px-5 py-4 font-mono text-xs leading-relaxed text-ink">
        ...
        {CALL_ROWS.map((r, i) => {
          ...
          return (
            <span key={i} className="block">
              ...
              <span style={{ color: 'var(--color-ink)' }}>{f.countStr}</span>
              <span style={{ color: pipeColor }}>{'  '}</span>
              <span style={{ color: 'var(--color-ink)' }}>{f.cpuStr}</span>
              <span style={{ color: pipeColor }}>{' '}</span>
              <span style={{ color: 'var(--color-ink)' }}>{f.elapsedStr}</span>
              <span style={{ color: pipeColor }}>{' '}</span>
              <span style={{ color: 'var(--color-ink)' }}>{f.diskStr}</span>
              <span style={{ color: pipeColor }}>{' '}</span>
              <span style={{ color: 'var(--color-ink)' }}>{f.queryStr}</span>
              <span style={{ color: pipeColor }}>{' '}</span>
              <span style={{ color: 'var(--color-ink)' }}>{f.currentStr}</span>
              <span style={{ color: pipeColor }}>{' '}</span>
              <span style={{ color: 'var(--color-ink)' }}>{f.rowsStr}</span>
            </span>
          )
        })}
        {'\n'}
        {extraLines.map((line, i) => (
          <span key={i} style={{ color: 'var(--color-ink-2)' }} className="block">{line}</span>
        ))}
      </pre>
      {/* 단계별 설명 패널 */}
      <div className="mt-4 rounded-panel border border-line bg-paper-sunk">
        <div className="flex items-center gap-2 border-b border-line px-4 py-2">
          <span className="text-xs font-bold text-ink-2">{lang === 'ko' ? '단계별 설명' : 'Step details'}</span>
        </div>
        <div className="divide-y divide-line">
          {CALL_ROWS.filter(r => !r.isTotal).map(r => {
            ...
            return (
              <div key={r.call} className="flex gap-3 px-4 py-2.5">
                <span className="mt-0.5 shrink-0 font-mono text-xs font-bold text-ink">{r.call}</span>
                <span className="text-xs leading-relaxed text-ink/70">{note}</span>
              </div>
            )
          })}
        </div>
...
export function PredicateInfoDisplay({ lang }: { lang: 'ko' | 'en' }) {
  ...
  const lines = isKo ? [
    { label: 'Plan hash value: 1234567890', color: 'var(--color-ink-3)' },
    { label: '', color: '' },
    { label: '-----------------------------------------------------', color: 'var(--color-ink-2)' },
    { label: '| Id  | Operation             | Name        | Rows |', color: 'var(--color-ink-3)' },
    { label: '-----------------------------------------------------', color: 'var(--color-ink-2)' },
    { label: '|   0 | SELECT STATEMENT      |             |   50 |', color: 'var(--color-blue)' },
    { label: '|*  1 |  HASH JOIN            |             |   50 |', color: 'var(--color-purple)' },
    { label: '|   2 |   TABLE ACCESS FULL   | DEPARTMENTS |   27 |', color: 'var(--color-amber)' },
    { label: '|*  3 |   INDEX RANGE SCAN    | EMP_DEPT_IX |  107 |', color: 'var(--color-green)' },
    { label: '-----------------------------------------------------', color: 'var(--color-ink-2)' },
    { label: '', color: '' },
    { label: 'Predicate Information (identified by operation id):', color: 'var(--color-amber)' },
    { label: '---------------------------------------------------', color: 'var(--color-ink-2)' },
    { label: '   1 - access("E"."DEPARTMENT_ID"="D"."DEPARTMENT_ID")', color: 'var(--color-amber)' },
    { label: '   3 - access("E"."DEPARTMENT_ID">50)', color: 'var(--color-amber)' },
  ] : [
    { label: 'Plan hash value: 1234567890', color: 'var(--color-ink-3)' },
    { label: '', color: '' },
    { label: '-----------------------------------------------------', color: 'var(--color-ink-2)' },
...
export function ColumnProjectionDisplay({ lang }: { lang: 'ko' | 'en' }) {
  ...
  const lines = isKo ? [
    { label: 'Column Projection Information (identified by operation id):', color: 'var(--color-amber)' },
    { label: '-----------------------------------------------------------', color: 'var(--color-ink-2)' },
    { label: '   1 - (#keys=1) "E"."EMPLOYEE_ID"[NUMBER,22], "D"."DEPARTMENT_NAME"[VARCHAR2,30]', color: 'var(--color-green)' },
    { label: '   2 - "D"."DEPARTMENT_ID"[NUMBER,22], "D"."DEPARTMENT_NAME"[VARCHAR2,30]', color: 'var(--color-green)' },
    { label: '   3 - "E"."ROWID"[ROWID,10], "E"."EMPLOYEE_ID"[NUMBER,22]', color: 'var(--color-green)' },
    { label: '', color: '' },
    { label: '-- #keys=1: 조인 키 컬럼 수', color: 'var(--color-ink-2)' },
    { label: '-- [NUMBER,22]: 데이터 타입과 길이', color: 'var(--color-ink-2)' },
  ] : [
    { label: 'Column Projection Information (identified by operation id):', color: 'var(--color-amber)' },
    { label: '-----------------------------------------------------------', color: 'var(--color-ink-2)' },
    { label: '   1 - (#keys=1) "E"."EMPLOYEE_ID"[NUMBER,22], "D"."DEPARTMENT_NAME"[VARCHAR2,30]', color: 'var(--color-green)' },
    ...
    { label: '   3 - "E"."ROWID"[ROWID,10], "E"."EMPLOYEE_ID"[NUMBER,22]', color: 'var(--color-green)' },
    { label: '', color: '' },
    { label: '-- #keys=1: number of join key columns', color: 'var(--color-ink-2)' },
    { label: '-- [NUMBER,22]: data type and length', color: 'var(--color-ink-2)' },
  ]
      ...
      <div className="overflow-x-auto rounded-panel border border-line bg-paper px-4 py-3">
        <table className="w-full font-mono text-[11px]">
          <tbody>
            {stats.map((s, i) => (
              <tr key={i}>
                <td className="pr-6 py-0.5 text-right text-ink w-12">{s.value}</td>
                <td className="pr-4 py-0.5 text-ink">{s.name}</td>
                <td className="py-0.5 text-ink-2 italic text-[10px]">{s.note}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  const blocks: Block[] = [
    {
      ...
      color: 'var(--color-blue)',
      lines: [
        { text: 'call     count       cpu    elapsed       disk      query    current       rows', color: 'var(--color-ink-3)' },
        { text: '------- ------  -------- ---------- ---------- ---------- ---------- ----------', color: pip },
        { text: 'Parse        1      0.00       0.01          0          0          0          0', color: 'var(--color-blue)' },
        { text: 'Execute      1      0.00       0.00          0          0          0          0', color: 'var(--color-green)' },
        { text: 'Fetch        2      0.01       0.03          3         14          0         10', color: 'var(--color-blue)' },
        { text: '------- ------  -------- ---------- ---------- ---------- ---------- ----------', color: pip },
        { text: 'total        4      0.01       0.04          3         14          0         10', color: 'var(--color-amber)' },
        { text: '', color: '' },
    ...
    },
    {
      ...
      color: 'var(--color-green)',
      lines: [
        { text: '| Id | Operation          | Name        | Starts | E-Rows | A-Rows |   A-Time   |  CR |  PR |', color: 'var(--color-ink-3)' },
        { text: '----+--------------------+-------------+--------+--------+--------+------------+-----+-----+', color: pip },
        { text: '|  0 | SELECT STATEMENT   |             |      1 |     50 |     50 | 00:00:00.02|  18 |   0 |', color: 'var(--color-blue)' },
        { text: '|  1 |  HASH JOIN         |             |      1 |     50 |     50 | 00:00:00.02|  18 |   0 |', color: 'var(--color-purple)' },
        { text: '|  2 |   TABLE ACCESS FULL| DEPARTMENTS |      1 |     27 |     27 | 00:00:00.01|   7 |   0 |', color: 'var(--color-amber)' },
        { text: '|  3 |   INDEX RANGE SCAN | EMP_DEPT_IX |      1 |    107 |    107 | 00:00:00.01|   3 |   0 |', color: 'var(--color-green)' },
        { text: '----+--------------------+-------------+--------+--------+--------+------------+-----+-----+', color: pip },
      ],
    },
    {
      ...
      color: 'var(--color-amber)',
      lines: [
        { text: 'Predicate Information (identified by operation id):', color: 'var(--color-amber)' },
        { text: '---------------------------------------------------', color: pip },
        { text: '   1 - access("E"."DEPARTMENT_ID"="D"."DEPARTMENT_ID")', color: 'var(--color-amber)' },
        { text: '   3 - access("E"."DEPARTMENT_ID">50)', color: 'var(--color-amber)' },
      ],
    },
    {
      ...
      color: 'var(--color-purple)',
      lines: [
        { text: 'Column Projection Information (identified by operation id):', color: 'var(--color-purple)' },
        { text: '----------------------------------------------------------', color: pip },
        { text: '   1 - (#keys=1) "D"."DEPARTMENT_NAME"[VARCHAR2,30],', color: 'var(--color-purple)' },
        { text: '       "E"."EMPLOYEE_ID"[NUMBER,22]', color: 'var(--color-purple)' },
        { text: '   2 - "D"."DEPARTMENT_ID"[NUMBER,22], "D"."DEPARTMENT_NAME"[VARCHAR2,30]', color: 'var(--color-purple)' },
        { text: '   3 - "E"."DEPARTMENT_ID"[NUMBER,22], "E"."EMPLOYEE_ID"[NUMBER,22]', color: 'var(--color-purple)' },
      ],
    },
    {
      ...
      color: 'var(--color-red)',
      lines: [
        { text: '         0  recursive calls', color: 'var(--color-red)' },
        { text: '         0  db block gets', color: 'var(--color-red)' },
        { text: '        18  consistent gets', color: 'var(--color-red)' },
        { text: '         0  physical reads', color: 'var(--color-red)' },
        { text: '         0  redo size', color: 'var(--color-red)' },
        { text: '        50  rows processed', color: 'var(--color-red)' },
      ],
    },
  ]

  return (
    <div className="my-4 overflow-x-auto rounded-panel border border-line bg-paper px-5 py-4 font-mono text-xs leading-relaxed">
      {blocks.map((block, bi) => (
        <div key={bi} className={bi > 0 ? 'mt-5' : ''}>
          {/* 영역 레이블 */}
          <div className="mb-1.5 flex items-center gap-2">
            <span className="inline-block h-2 w-2 rounded-full bg-line-2" />
            <span className="text-[11px] font-bold text-ink-2">{block.label}</span>
          </div>
          {/* 출력 내용 */}
          <div>
        ...
        <button
          ...
          className={cn(
            ...
            active === m.key
              ...
              : 'border-line-2 bg-paper text-ink hover:border-blue/50',
          )}
        >
          {m.label}
        </button>
