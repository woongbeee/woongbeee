...
const T = {
  ko: {
    ...
    howTitle: '작동 방식',
    step1Title: '① 인덱스 세그먼트 직접 접근',
    step1Desc: 'Root/Branch를 거치지 않고 인덱스 세그먼트의 블록을 직접 읽습니다. 트리 탐색 비용이 전혀 없습니다.',
    step2Title: '② 멀티블록 I/O로 한꺼번에 읽기',
    step2Desc: 'db_file_multiblock_read_count 설정에 따라 한 번의 I/O로 여러 블록을 한꺼번에 읽습니다. 물리적 저장 순서대로 읽으므로 결과 순서는 보장되지 않습니다.',
    ...
    traits: [
      ...
      { icon: '📊', title: '병렬 처리와 궁합이 좋음', desc: '파티션 여부와 상관없이 병렬 쿼리를 사용하면 여러 PX 서버가 인덱스 세그먼트를 나눠 동시에 읽습니다. Full Scan은 단일 블록 I/O 특성상 병렬 처리 효과가 제한적이지만, Fast Full Scan은 멀티블록 I/O와 병렬 처리를 함께 활용해 극적인 속도 향상을 얻을 수 있습니다.' },
    ],
    parallelNote: 'Fast Full Scan은 인덱스가 파티션되어 있지 않아도 병렬 쿼리가 가능합니다. 오라클은 인덱스 세그먼트를 여러 범위(granule)로 나눠 각 PX 서버에 할당하므로, 파티션 프루닝 없이도 병렬 I/O의 이점을 그대로 얻을 수 있습니다.',
    ...
    hintTitle: 'Index Fast Full Scan이 쓰이는 때',
    hintNote: '인덱스에 포함된 컬럼만 필요하고 결과 순서가 상관없을 때 선택됩니다. 옵티마이저가 느린 플랜을 선택할 경우 INDEX_FFS 힌트로 강제할 수 있습니다.',
    examples: [
      { badge: 'COUNT(*)', badgeColor: 'violet' as const,
        desc: 'COUNT(*)는 행이 존재하는지만 확인하면 되므로 어떤 컬럼 값도 필요하지 않습니다. PK 등 임의의 인덱스로 전체 인덱스 세그먼트를 멀티블록 I/O로 읽기만 하면 됩니다. 테이블 블록에 전혀 접근하지 않으므로 가장 빠른 전체 카운트 방법입니다.',
        sql: `SELECT COUNT(*)\nFROM employees;\n-- PK 등 어떤 인덱스든 활용 가능\n-- 인덱스 세그먼트를 멀티블록 I/O로 직접 읽음\n-- 테이블 접근 전혀 없음\n-- 실행 계획: INDEX FAST FULL SCAN` },
      { badge: '집계 함수', badgeColor: 'violet' as const,
        desc: 'MAX, MIN, SUM은 정렬된 순서가 필요 없습니다. 인덱스 세그먼트 전체를 멀티블록 I/O로 읽으면 되므로, Full Scan(1블록씩)보다 I/O 횟수가 훨씬 적습니다. ORDER BY가 없는 집계에서는 항상 Fast Full Scan이 Full Scan보다 유리합니다.',
      ...
      { badge: 'INDEX_FFS 힌트', badgeColor: 'blue' as const,
        desc: '옵티마이저가 Full Scan이나 Full Table Scan을 선택했을 때 Fast Full Scan으로 강제합니다. 정렬이 불필요한 쿼리인데 통계가 오래되어 잘못된 플랜이 나올 때 유용합니다.',
      ...
      { badge: 'PARALLEL + INDEX_FFS', badgeColor: 'blue' as const,
        desc: '병렬 처리와 Fast Full Scan을 함께 쓰면 대용량 집계에서 가장 빠른 속도를 얻을 수 있습니다. 오라클은 인덱스 세그먼트를 granule 단위로 분할해 각 PX 서버에 할당합니다. 인덱스가 파티션되어 있지 않아도 세그먼트 레벨에서 분할이 가능하므로 병렬 처리가 적용됩니다.',
